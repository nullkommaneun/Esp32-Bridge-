...
class AudioSensor {
    ...
    async start() {
        ...
        } catch(e) { log("Mikrofon Fehler (Ignoriert): "+e.message, "error"); }
    }
...
class AnomalyDetector {
    ...
    prep(rssi, sens, rate, audio) {
        // Normalisierung aller Inputs auf 0.0 - 1.0
    ...
    async detect(rssi, sens, rate, audio) {
        ...
        // Dynamische Schwelle: Toleranz bei Lärm und Bewegung erhöhen
    ...
    async forceLearn(vecs) {
        ...
        log(`🎓 Lerne ${vecs.length} Muster als SICHER...`, "learning");
        const d = tf.tensor2d(vecs);
        await this.model.fit(d, d, {epochs:10, shuffle:true}); // Intensives Training
        d.dispose();
...
class StaplerApp {
    ...
    loop() {
        ...
        if (bestDev) {
            ...
            document.getElementById('tel-rssi').innerText = bestDev.avg.toFixed(1) + " dB";
            document.getElementById('tel-rssi').style.color = bestDev.avg > -60 ? "#ff0055" : "#fff";
            
            if (bestDev.buf.length < 20) {
                document.getElementById('tel-buf').innerText = `LADEN (${bestDev.buf.length})`;
                document.getElementById('tel-buf').style.color = "#ffaa00";
                document.getElementById('tel-status').innerText = "SAMMLE DATEN";
            } else {
                document.getElementById('tel-buf').innerText = "VOLL";
                document.getElementById('tel-buf').style.color = "#00c853";
                document.getElementById('tel-status').innerText = "ANALYSIERE...";
            }

            document.getElementById('tel-rate').innerText = bestDev.rate + "/s";
            ...
            if (bestDev.loss > bestDev.lim) {
                document.getElementById('tel-loss').style.color = "#ff0055";
                document.getElementById('tel-status').innerText = "⚠️ ANOMALIE";
            } else {
                document.getElementById('tel-loss').style.color = "#00d2ff";
                document.getElementById('tel-status').innerText = "✅ NORMAL";
            }
        } else {
            document.getElementById('tel-mac').innerText = "--";
        ...
        document.getElementById('tel-audio-dbg').innerText = aud.toFixed(0);
    ...
    reportFalseAlarm() {
        ...
        setTimeout(() => { btn.innerText = "✋ DAS WAR EIN FEHLALARM! (LERNEN)"; }, 2000);
    }
